...
export default function StatementOfAccountPage() {
  ...
  const handlePrint = () => {
      ...
      // For a more robust solution, a dedicated print-friendly CSS file would be needed.
      window.print();
  }
    ...
    <Card>
      <CardHeader>
        <div className="flex justify-between items-center">
             ...
             <Button onClick={handlePrint} variant="outline" size="sm" className="gap-1 print-hidden"><Printer className="h-4 w-4"/> Print</Button>
        </div>
      </CardHeader>
      <CardContent>
         ...
         <div className="grid grid-cols-2 gap-4 mb-4 print:grid-cols-2">
            <p><strong>M/s:</strong> {customer}</p>
            <p className="text-right"><strong>Date:</strong> {new Date(statementDate).toLocaleDateString('en-GB')}</p>
        </div>
                ...
                <Table>
                    <TableHeader>
                        <TableRow>
                            ...
                            <TableHead className="w-8 h-8 p-1 print-hidden"></TableHead>
                        </TableRow>
                    </TableHeader>
                    <TableBody>
                        {creditRows.map((tx, i) => (
                            <TableRow key={`credit-${i}`} className="h-6">
                                ...
                                <TableCell className="p-0.5 print-hidden"><Button size="icon" variant="ghost" className="h-6 w-6" onClick={() => removeCreditRow(i)}><Trash2 className="h-3 w-3 text-destructive"/></Button></TableCell>
                            </TableRow>
                        ))}
                    </TableBody>
                </Table>
                 <Button variant="outline" size="sm" onClick={addCreditRow} className="m-1 print-hidden"><PlusCircle className="h-4 w-4 mr-2"/>Add Credit Row</Button>
            </div>
            {/* DEBIT SIDE */}
            <div className="pl-1">
                    ...
                    <TableBody>
                        {debitRows.map((tx, i) => (
                            <TableRow key={`debit-${i}`} className="h-6">
                                ...
                                <TableCell className="p-0.5 print-hidden"><Button size="icon" variant="ghost" className="h-6 w-6" onClick={() => removeDebitRow(i)}><Trash2 className="h-3 w-3 text-destructive"/></Button></TableCell>
                            </TableRow>
                        ))}
                    </TableBody>
                </Table>
                 <Button variant="outline" size="sm" onClick={addDebitRow} className="m-1 print-hidden"><PlusCircle className="h-4 w-4 mr-2"/>Add Debit Row</Button>
            </div>
            {/* TOTALS */}
                ...
                <Table>
                    <TableBody>
                        <TableRow className="h-6 font-bold">
                            ...
                            <TableCell className="w-8 p-1 print-hidden"></TableCell>
                        </TableRow>
                    </TableBody>
                </Table>
            </div>
      ...
      <CardFooter className="flex-col items-center gap-4">
        ...
        <Button onClick={handleSave} className="w-full max-w-sm print-hidden">Save Statement</Button>
      </CardFooter>
    </Card>
  );
}
