...
function calculateCommissionDeductions(grossSale) {
  ...
  const commissionAmount = roundAmount(
    grossSaleValue * COMMISSION_RATE
  );

  const securityCharges = roundAmount(
    grossSaleValue * SECURITY_CHARGES_RATE
  );

  const totalCommissionRelatedExpenses = roundAmount(
    commissionAmount + securityCharges
  );
...
function calculateInvoiceTotals({
  ...
  const {
    commissionAmount,
    securityCharges,
  } = calculateCommissionDeductions(grossSaleValue);
  ...
  const netSale = roundAmount(
    grossSaleValue - totalExpenses
  );
...
function normalizeInvoiceData(billData = {}) {
  const entries = Array.isArray(billData.entries)
    ? billData.entries
    : [];
  ...
  const pattiQty = entries
    .filter((entry) => entry.type === 'Patti')
    .reduce(
      (sum, entry) =>
        sum + (Number(entry.qty) || 0),
      0
  ...
  const dabbaQty = entries
    .filter((entry) => entry.type === 'Dabba')
    .reduce(
      (sum, entry) =>
        sum + (Number(entry.qty) || 0),
      0
  ...
  const freight = roundAmount(
    billData.freight ||
    billData.totals?.freight ||
    0
  );

  const postage = normalizePostage(billData.totals?.postage);

  const otherExpenses = roundAmount(
    billData.totals?.otherExpenses ?? 0
  );

  const calculated = calculateInvoiceTotals({
    grossSale:
      billData.totals?.grossSale ?? grossSale,
    totalQty,
    freight,
    postage,
    otherExpenses,
