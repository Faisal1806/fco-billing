...
export default function PaymentPage({ params }: { params: { id: string } }) {
    ...
    useEffect(() => {
        ...
        const fetchReceipt = async () => {
            ...
            if (isPublicView) {
                // getDocument expects a single document path string
    ...
    const A4Layout = () => (
         <div className="w-[105mm] min-h-[148mm] mx-auto bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-200 shadow-lg print:shadow-none p-6 flex flex-col">
                <header className="bg-gradient-to-r from-yellow-400 via-orange-500 to-amber-600 text-white p-4 rounded-t-xl shadow-md flex justify-between items-center">
                    <div className="text-sm font-bold">ðŸŽ F.Co</div>
                ...
                <main className="flex-grow bg-white dark:bg-gray-800 p-4 -mt-2 rounded-b-xl shadow-lg">
                    ...
                    <div className="grid grid-cols-2 gap-4 text-xs font-semibold">
                        <div>
                           <p>Freight Paid Rs: {freightPaid > 0 ? `â‚¹${freightPaid.toFixed(2)}` : ''}</p>
        ...
        <div className="w-[80mm] bg-white text-black p-2 font-mono text-[10px] leading-tight">
            <header className="text-center space-y-1">
             ...
             <div className="text-xs mt-2 space-y-1 border-t border-dashed pt-2">
                <p>Freight Paid: {freightPaid > 0 ? `â‚¹${freightPaid.toFixed(2)}` : 'N/A'}</p>
                <p>Watak Ready On: {wattakReadyOn}</p>
            </div>
        ...
        <div className="bg-gray-100 dark:bg-gray-900 font-sans print:bg-white flex flex-col md:flex-row gap-8 justify-center p-4 md:p-8">
            <style jsx global>{`
                ...
                @media print {
                    ...
                    @page {
                        size: ${printStyle === 'a4' ? (effectiveOrientation === 'landscape' ? 'A6 landscape' : 'A6 portrait') : '80mm 297mm'};
                        margin: 1mm;
                    }
                }
            `}</style>
